...
#        self.tableView.setSelectionBehavior (QAbstractItemView.SelectRows )
    ...
    def calculate_ms(self, x, y):
        # Split time strings into their components
        xhour, xmin, xsec, xms = map(int, x.split(":"))
        yhour, ymin, ysec, yms = map(int, y.split(":"))

        # Calculate total milliseconds for x and y
        total_ms_x = xms + xsec*1000 + xmin*60*1000 + xhour*60*60*1000
        total_ms_y = yms + ysec*1000 + ymin*60*1000 + yhour*60*60*1000

        # Return the difference
        return total_ms_x - total_ms_y
    ...
    def processCsv_1(self):
        ...
        left_side_on_count = 0
        right_side_on_count = 0

        houselight_status = False
        houselight_nosepokec_count = 0
        houselight_count = 0


        omm_count = 0
        trial_count = 0
        center_omission_count = 0
        side_omission_count = 0
        left_side_assigned_count = 0
        right_side_assigned_count = 0
        C_trigger_poke_time = 0
        trial_status = False
        left_on_poke_status = False
        left_omission_count = 0
        left_on_poke_time = 0
        center_status = False

        center_nosepoke_B_count = 0
        center_nosepoke_D_count = 0
        ...
        for x in self.raw[0]:
            ...
            ## find number of center omission
            if "center omission" in event:
                center_omission_count +=1

            ## find number of center omission
            if "side omission" in event:
                side_omission_count +=1

            ## find houselight on to nosepoke_C in
            if "trial start" in event:
                 houselight_count += 1
                 houselight_status = True
                 temp_houselight = time
                 trial_status = True
            if "trial end" in event:
                trial_status = False
            if "center triggered" in event and houselight_status and trial_status:
                 houselight_nosepokec_count += 1
                 houselight_status = False
                 temp = self.calculate_ms(time, temp_houselight)
                 print("temp calc", temp, time, temp_houselight)
                 C_trigger_poke_time = int(C_trigger_poke_time) + temp
                 temp_houselight = 0


            ## find left side assigned to nosepoke B on time
            if "left side assigned" in event:
                left_side_on_count += 1
                left_on_poke_status = True
                temp_t_left = time
            if "nosepoke_B in" in event and left_on_poke_status and trial_status:
                left_on_poke_status = False
                print(">>>> [Debug] left side assigned at time, ", temp_t_left, "nosepoke B triggered, ", time)
                temp = self.calculate_ms(time, temp_t_left)
                left_on_poke_time = int(left_on_poke_time) + temp
                temp_t_B_light = 0
            elif "omission" in event and left_on_poke_status and trial_status:
                left_omission_count += 1
                B_on_poke_status = False
                temp_t_B_light = 0

            ## find right side assigned to nosepoke D on time
            if "right side assigned" in event:
                right_side_on_count  += 1
                right_on_poke_status = True
                temp_t_right = time
            if "nosepoke_D in" in event and D_on_poke_status:
                D_on_poke_status = False
                temp = self.calculate_ms(time, temp_t_right)
                left_on_poke_time = int(left_on_poke_time) + temp
                temp_t_D_light = 0
            ...
            if "center triggered" in event:
                center_status = True
            if "left side assigned" in event and center_status:
                center_status = False
                if center_nosepoke_D_count > 0:
                    left_unwanted_D += 1
                center_nosepoke_D_count = 0
                center_nosepoke_B_count = 0
            if "right side assigned" in event and center_status:
                center_status = False
                if center_nosepoke_B_count > 0:
                    right_unwanted_B += 1
                center_nosepoke_D_count = 0
                center_nosepoke_B_count = 0
            if "nosepoke_B in" in event and center_status:
                center_nosepoke_B_count += 1
            if "nosepoke_D in" in event and center_status:
                center_nosepoke_D_count += 1
        ...
        print("center omission,", center_omission_count)
        print("side omission,", side_omission_count)
        print("trial start num,", trial_count)

        print("right side assigned,", right_side_on_count)
        print("left side assigned,", left_side_on_count)
        print("houselight_count,", houselight_count)
        ...
        print("\n======= Reporting houselight - nosepoke C trigger =======")
        print("C_trigger_poke_time, ", C_trigger_poke_time)
        print("houselight_nosepokec_count, ", houselight_nosepokec_count)
        try:
            print("houselight on --> nosepoke C trigger avg time, ", C_trigger_poke_time/houselight_nosepokec_count)
            print("houselight on --> nosepoke C trigger number", houselight_nosepokec_count)
        except:
            print("No <houselight on and nokepoke C trigger> case. Or analysis error")

        print("\n====== Reporting unwanted B or D ==============")
        print("right assigned and unwated B, ", right_unwanted_B)
        print("left assigned and unwated D, ", left_unwanted_D)
