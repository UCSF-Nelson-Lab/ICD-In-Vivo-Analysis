...
#        self.tableView.setSelectionMode(QAbstractItemView.MultiSelection)
    ...
    def calculate_ms(self, x, y): # x is larger than y
        xhour = int(x.split(":")[0])
        xmin = int(x.split(":")[1])
        xsec = int(x.split(":")[2])
        xms = int(x.split(":")[3])
        ...
        if xms > yms:
            diff = xms - yms
            if xsec > ysec and xmin > ymin:
                diff = diff + (xsec - ysec)*1000 + (xmin - ymin)*60*1000
            else:
                diff_sec = ((xmin - ymin)*60 + xsec) - ysec
                diff = diff + diff_sec*1000
        else:
            diff = 0
            if xsec > ysec:
                diff = ((xsec-ysec)*1000 + xms)- yms
            elif xmin > ymin:
                diff_sec = ((xmin - ymin)*60 + xsec) - ysec
                diff = (diff_sec*1000 + xms)- yms
        return diff
    ...
    def processCsv_1(self):
        ...
        current_block = ""
        latency_time = 0
        trial_npc_lock = False
        centri_npc_lock = False
        ...
        for x in self.raw[0]:
            ...
            if "B3" in event:
                ...
                result[current_block]["trial-npc_time"] = 0
                result[current_block]["trial-npc_n"] = 0
                result[current_block]["centri-npc_time"] = 0
                result[current_block]["centri-npc_n"] = 0
                result[current_block]["small_r"] = 0
                result[current_block]["large_r"] = 0
                result[current_block]["center_omission"] = 0
                result[current_block]["side_omission"] = 0
                result[current_block]["none_omission"] = 0
                result[current_block]["total_latency_start_centri"] = 0
                result[current_block]["total_latency_centri_Bin"] = 0
                result[current_block]["total_latency_centri_Din"] = 0
                result[current_block]["large_reward_set"] = []
                result[current_block]["small_reward_set"] = []
                result[current_block]["ommision_set"] = []
            ...
            if "B1" in event or "B2" in event or "B4" in event or "B5" in event or "B7" in event or "B8" in event or "B10" in event or "B11" in event:
                block_lock = False
            ...
            if "trial start" in event:
                ...
                reward_type = 0
                trial_count += 1
            if "trial end" in event:
                trial_status = False
                trial_npc_lock = False
                centri_npc_lock = False


            if block_lock and "s reward delivery" in event:
                result[current_block]["small_r"] += 1
                latency_lock = True
                reward_status = True
                reward_type = 1
                result[current_block]["none_omission"] += 1
                result[current_block]["small_reward_set"].append(trial_count)

            if block_lock and "l reward delivery" in event:
                result[current_block]["large_r"] += 1
                latency_lock = True
                reward_status = True
                reward_type = 2
                result[current_block]["none_omission"] += 1
                result[current_block]["large_reward_set"].append(trial_count)

            if block_lock and "center omi" in event:
                result[current_block]["center_omission"] += 1
                latency_lock = False
                result[current_block]["ommision_set"].append(trial_count)

            if block_lock and "side omi" in event:
                result[current_block]["side_omission"] += 1
                latency_lock = False
                latency_lock2 = False
                result[current_block]["ommision_set"].append(trial_count)

            if block_lock and "trial start" in event:
                trial_npc_lock = True
                latency_lock = True
                temp_time = time

            if block_lock and trial_npc_lock and trial_status:
                result[current_block]["trial-npc_n"] += 1
                trial_npc_lock = False
                temp = self.calculate_ms(time, temp_time)
                result[current_block]["trial-npc_time"] = int(result[current_block]["trial-npc_time"]) + temp

            if latency_lock and "trial start" in event:
                latency_temp = time
            if latency_lock and "cen tri" in event:
                latency_time = self.calculate_ms(time, latency_temp)
                latency_lock2 = True
                latency_temp2 = time
            if latency_lock2 and  "np_B in" in event and latency_calc_lock is False:
                latency_Bin = self.calculate_ms(time, latency_temp2)
                latency_calc_lock = True
            if latency_lock2 and  "np_D in" in event and latency_calc_lock is False:
                latency_Din = self.calculate_ms(time, latency_temp2)
                latency_calc_lock = True
            if latency_lock and "timeout" in event:
                result[current_block]["total_latency_start_centri"] = int(result[current_block]["total_latency_start_centri"]) + latency_time
                latency_lock = False
                latency_time = 0
            if latency_lock2 and "timeout" in event:
                if latency_Bin is not 0:
                    result[current_block]["total_latency_centri_Bin"] = int(result[current_block]["total_latency_centri_Bin"]) + latency_Bin
                if latency_Din is not 0:
                    result[current_block]["total_latency_centri_Din"] = int(result[current_block]["total_latency_centri_Din"]) + latency_Din
                latency_Bin = 0
                latency_Din = 0
                latency_calc_lock = False
                latency_lock2 = False

            if reward_status and not first_cin and "np_C in" in event:
                first_cin = True
        ...
        print("\nBlock,", "B3 >>\n", result["B3"])
        try:
            print("\nTrial start to cen tri avg time: ", int(result["B3"]["total_latency_start_centri"])/int(result["B3"]["none_omission"]))
        except:
            print("\nNo none_omission")
        try:
            print("\nIF << nosepoke B --> Large reward >> \ncen tri to B avg time: ", int(result["B3"]["total_latency_centri_Bin"])/int(result["B3"]["large_r"]))
        except:
            print("\nNo large reward")
        try:
            print("\nIF << nosepoke D --> Small reward >> \ncen tri to D avg time: ", int(result["B3"]["total_latency_centri_Din"])/int(result["B3"]["small_r"]))
        except:
            print("\nNo small reward")
        try:
            print("\nIF << nosepoke B --> Small reward >> \ncen tri to B avg time: ", int(result["B3"]["total_latency_centri_Bin"])/int(result["B3"]["small_r"]))
        except:
            print("\nNo small reward")
        try:
            print("\nIF << nosepoke D --> Large reward >> \ncen tri to D avg time: ", int(result["B3"]["total_latency_centri_Din"])/int(result["B3"]["large_r"]))
            print()
        except:
            print("\nNo large reward")
        try:
            print("\nOverall latency B and D avg time",(int(result["B3"]["total_latency_centri_Bin"]) + int(result["B3"]["total_latency_centri_Din"]))//int(result["B3"]["none_omission"]))
        except:
            print("\nNo none_omission")

        print("\nBlock,", "B6 >>\n", result["B6"])
        try:
            print("\nTrial start to cen tri avg time: ", int(result["B6"]["total_latency_start_centri"])/int(result["B6"]["none_omission"]))
        except:
            print("\nNo none_omission")
        try:
            print("\nIF << nosepoke B --> Large reward >> \ncen tri to B avg time: ", int(result["B6"]["total_latency_centri_Bin"])/int(result["B6"]["large_r"]))
        except:
            print("\nNo large reward")
        try:
            print("\nIF << nosepoke D --> Small reward >> \ncen tri to D avg time: ", int(result["B6"]["total_latency_centri_Din"])/int(result["B6"]["small_r"]))
        except:
            print("\nNo small reward")
        try:
            print("\nIF << nosepoke B --> Small reward >> \ncen tri to B avg time: ", int(result["B6"]["total_latency_centri_Bin"])/int(result["B6"]["small_r"]))
        except:
            print("\nNo small reward")
        try:
            print("\nIF << nosepoke D --> Large reward >> \ncen tri to D avg time: ", int(result["B6"]["total_latency_centri_Din"])/int(result["B6"]["large_r"]))
        except:
            print("\nNo large reward")
        try:
            print("\nOverall latency B and D avg time",(int(result["B6"]["total_latency_centri_Bin"]) + int(result["B6"]["total_latency_centri_Din"]))//int(result["B6"]["none_omission"]))
        except:
            print("\nNo none_omission")
        ...
        try:
            print("\nTrial start to cen tri avg time: ", int(result["B9"]["total_latency_start_centri"])/int(result["B9"]["none_omission"]))
        except:
            print("\nNo none_omission")
        try:
            print("\nIF << nosepoke B --> Large reward >> \ncen tri to B avg time: ", int(result["B9"]["total_latency_centri_Bin"])/int(result["B9"]["large_r"]))
        except:
            print("\nNo large reward")
        try:
            print("\nIF << nosepoke D --> Small reward >> \ncen tri to D avg time: ", int(result["B9"]["total_latency_centri_Din"])/int(result["B9"]["small_r"]))
        except:
            print("\nNo small reward")
        try:
            print("\nIF << nosepoke B --> Small reward >> \ncen tri to B avg time: ", int(result["B9"]["total_latency_centri_Bin"])/int(result["B9"]["small_r"]))
        except:
            print("\nNo small reward")
        try:
            print("\nIF << nosepoke D --> Large reward >> \ncen tri to D avg time: ", int(result["B9"]["total_latency_centri_Din"])/int(result["B9"]["large_r"]))
        except:
            print("\nNo large reward")
        try:
            print("\nOverall latency B and D avg time",(int(result["B9"]["total_latency_centri_Bin"]) + int(result["B9"]["total_latency_centri_Din"]))//int(result["B9"]["none_omission"]))
        except:
            print("\nNo none_omission")
        ...
        try:
            print("\nBlock,", "B12 >>\n", result["B12"])
            try:
                print("\nTrial start to cen tri avg time: ", int(result["B12"]["total_latency_start_centri"])/int(result["B12"]["none_omission"]))
            except:
                print("\nNo none_omission")
            try:
                print("\nIF << nosepoke B --> Large reward >> \ncen tri to B avg time: ", int(result["B12"]["total_latency_centri_Bin"])/int(result["B12"]["large_r"]))
            except:
                print("\nNo large reward")
            try:
                print("\nIF << nosepoke D --> Small reward >> \ncen tri to D avg time: ", int(result["B12"]["total_latency_centri_Din"])/int(result["B12"]["small_r"]))
            except:
                print("\nNo small reward")
            try:
                print("\nIF << nosepoke B --> Small reward >> \ncen tri to B avg time: ", int(result["B12"]["total_latency_centri_Bin"])/int(result["B12"]["small_r"]))
            except:
                print("\nNo small reward")
            try:
                print("\nIF << nosepoke D --> Large reward >> \ncen tri to D avg time: ", int(result["B12"]["total_latency_centri_Din"])/int(result["B12"]["large_r"]))
            except:
                print("\nNo large reward")
            try:
                print("\nOverall latency B and D avg time",(int(result["B12"]["total_latency_centri_Bin"]) + int(result["B12"]["total_latency_centri_Din"]))//int(result["B12"]["none_omission"]))
            except:
                print("\nNo none_omission")
        ...
        try:
            print("\nBlock,", "B15 >>\n", result["B15"])
            try:
                print("\nTrial start to cen tri avg time: ", int(result["B15"]["total_latency_start_centri"])/int(result["B15"]["none_omission"]))
            except:
                print("\nNo none_omission")
            try:
                print("\nIF << nosepoke B --> Large reward >> \ncen tri to B avg time: ", int(result["B15"]["total_latency_centri_Bin"])/int(result["B15"]["large_r"]))
            except:
                print("\nNo large reward")
            try:
                print("\nIF << nosepoke D --> Small reward >> \ncen tri to D avg time: ", int(result["B15"]["total_latency_centri_Din"])/int(result["B15"]["small_r"]))
            except:
                print("\nNo small reward")
            try:
                print("\nIF << nosepoke B --> Small reward >> \ncen tri to B avg time: ", int(result["B15"]["total_latency_centri_Bin"])/int(result["B15"]["small_r"]))
            except:
                print("\nNo small reward")
            try:
                print("\nIF << nosepoke D --> Large reward >> \ncen tri to D avg time: ", int(result["B15"]["total_latency_centri_Din"])/int(result["B15"]["large_r"]))
            except:
                print("\nNo large reward")
            try:
                print("\nOverall latency B and D avg time",(int(result["B15"]["total_latency_centri_Bin"]) + int(result["B15"]["total_latency_centri_Din"]))//int(result["B15"]["none_omission"]))
            except:
                print("\nNo none_omission")
        ...
        small_r_total = result["B3"]["small_r"] + result["B6"]["small_r"] + result["B9"]["small_r"]
        large_r_total = result["B3"]["large_r"] + result["B6"]["large_r"] + result["B9"]["large_r"]

        try:
            with open('fileB3.csv', 'w') as f:
                for key in result["B3"].keys():
                    f.write("%s,%s\n"%(key,result["B3"][key]))
            with open('fileB6.csv', 'w') as f:
                for key in result["B6"].keys():
                    f.write("%s,%s\n"%(key,result["B6"][key]))
            with open('fileB9.csv', 'w') as f:
                for key in result["B9"].keys():
                    f.write("%s,%s\n"%(key,result["B9"][key]))
        except:
            print("Cannot save files B3 B6 B9")
        try:
            with open('fileB12.csv', 'w') as f:
                for key in result["B12"].keys():
                    f.write("%s,%s\n"%(key,result["B12"][key]))
        except:
            print("Cannot save files B12")
